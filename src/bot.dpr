...
begin
	...
	if NOT IsClient then if BotsDllArray.FilesCount>0 then for i:=0 to BotsDllArray.FilesCount-1 do if HaveBot(i) AND (@EXTERNAL_DLL_MainLoop[i]<>nil) AND BotsDllArray.StatusOnOff[i] then EXTERNAL_DLL_MainLoop[i]();
...
procedure P_DLL_PatchBot(DXID : word; h : BYTE; l : BYTE);
var hiparam : single,
loparam : single;
begin
	hiparam := h;
